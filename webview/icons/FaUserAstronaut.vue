<template>
    <svg
        width="55"
        height="55"
        viewBox="0 0 386 382"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink"
    >
        <rect width="385.517" height="381.086" fill="url(#pattern0_27_13)" />
        <defs>
            <pattern id="pattern0_27_13" patternContentUnits="objectBoundingBox" width="1" height="1">
                <use xlink:href="#image0_27_13" transform="matrix(0.0109834 0 0 0.0111111 0.00574714 0)" />
            </pattern>
            <image
                id="image0_27_13"
                width="90"
                height="90"
                xlink:href="data:image/png;base64,"
            />
        </defs>
    </svg>
</template>
